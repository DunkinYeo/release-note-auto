...
### GitHub Actions (권장) — 태그 push 시 자동 실행

```bash
# 1. config 편집
vi config/release_config.yaml

# 2. commit + tag + push → Actions 자동 실행 + Slack 알림
...
```bash
make run       # 의존성 자동 설치 + 릴리즈노트 생성
make open      # 생성된 PDF 바로 열기 (macOS)
make clean     # output/ 결과물 전체 삭제
```
